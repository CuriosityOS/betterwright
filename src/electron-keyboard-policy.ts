...
export class BetterwrightKeyboardPolicy {
  ...
  check(params: Record<string, UntrustedValue>): void {
    const reject = (): never => {
    ...
    for (const field of ["key", "code", "text", "unmodifiedText"]) {
      ...
      if (value !== undefined && !isString(value)) reject();
      if (isString(value) && value) keys.push(normalizeKey(value));
    }
    const virtual = params.windowsVirtualKeyCode;
    if (virtual !== undefined) {
      if (!isNumber(virtual) || !Number.isInteger(virtual) || virtual < 0 || virtual > 255)
        reject();
      if (isNumber(virtual)) {
        const key =
    ...
    if (
      ...
      (!Array.isArray(params.commands) ||
        params.commands.some(
          (command) => !isString(command) || !SAFE_COMMANDS.has(command),
        ))
    )
      reject();
    if (!isNumber(modifiers)) reject();
    let effectiveModifiers = Number(modifiers);
